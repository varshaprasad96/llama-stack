/*
Llama Stack Specification

This is the specification of the Llama Stack that provides                 a set of endpoints and their corresponding interfaces that are tailored to                 best leverage Llama Models.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the QueryTracesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QueryTracesRequest{}

// QueryTracesRequest struct for QueryTracesRequest
type QueryTracesRequest struct {
	AttributeFilters []QueryCondition `json:"attribute_filters,omitempty"`
	Limit *int32 `json:"limit,omitempty"`
	Offset *int32 `json:"offset,omitempty"`
	OrderBy []string `json:"order_by,omitempty"`
}

// NewQueryTracesRequest instantiates a new QueryTracesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQueryTracesRequest() *QueryTracesRequest {
	this := QueryTracesRequest{}
	return &this
}

// NewQueryTracesRequestWithDefaults instantiates a new QueryTracesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQueryTracesRequestWithDefaults() *QueryTracesRequest {
	this := QueryTracesRequest{}
	return &this
}

// GetAttributeFilters returns the AttributeFilters field value if set, zero value otherwise.
func (o *QueryTracesRequest) GetAttributeFilters() []QueryCondition {
	if o == nil || IsNil(o.AttributeFilters) {
		var ret []QueryCondition
		return ret
	}
	return o.AttributeFilters
}

// GetAttributeFiltersOk returns a tuple with the AttributeFilters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryTracesRequest) GetAttributeFiltersOk() ([]QueryCondition, bool) {
	if o == nil || IsNil(o.AttributeFilters) {
		return nil, false
	}
	return o.AttributeFilters, true
}

// HasAttributeFilters returns a boolean if a field has been set.
func (o *QueryTracesRequest) HasAttributeFilters() bool {
	if o != nil && !IsNil(o.AttributeFilters) {
		return true
	}

	return false
}

// SetAttributeFilters gets a reference to the given []QueryCondition and assigns it to the AttributeFilters field.
func (o *QueryTracesRequest) SetAttributeFilters(v []QueryCondition) {
	o.AttributeFilters = v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *QueryTracesRequest) GetLimit() int32 {
	if o == nil || IsNil(o.Limit) {
		var ret int32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryTracesRequest) GetLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *QueryTracesRequest) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int32 and assigns it to the Limit field.
func (o *QueryTracesRequest) SetLimit(v int32) {
	o.Limit = &v
}

// GetOffset returns the Offset field value if set, zero value otherwise.
func (o *QueryTracesRequest) GetOffset() int32 {
	if o == nil || IsNil(o.Offset) {
		var ret int32
		return ret
	}
	return *o.Offset
}

// GetOffsetOk returns a tuple with the Offset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryTracesRequest) GetOffsetOk() (*int32, bool) {
	if o == nil || IsNil(o.Offset) {
		return nil, false
	}
	return o.Offset, true
}

// HasOffset returns a boolean if a field has been set.
func (o *QueryTracesRequest) HasOffset() bool {
	if o != nil && !IsNil(o.Offset) {
		return true
	}

	return false
}

// SetOffset gets a reference to the given int32 and assigns it to the Offset field.
func (o *QueryTracesRequest) SetOffset(v int32) {
	o.Offset = &v
}

// GetOrderBy returns the OrderBy field value if set, zero value otherwise.
func (o *QueryTracesRequest) GetOrderBy() []string {
	if o == nil || IsNil(o.OrderBy) {
		var ret []string
		return ret
	}
	return o.OrderBy
}

// GetOrderByOk returns a tuple with the OrderBy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *QueryTracesRequest) GetOrderByOk() ([]string, bool) {
	if o == nil || IsNil(o.OrderBy) {
		return nil, false
	}
	return o.OrderBy, true
}

// HasOrderBy returns a boolean if a field has been set.
func (o *QueryTracesRequest) HasOrderBy() bool {
	if o != nil && !IsNil(o.OrderBy) {
		return true
	}

	return false
}

// SetOrderBy gets a reference to the given []string and assigns it to the OrderBy field.
func (o *QueryTracesRequest) SetOrderBy(v []string) {
	o.OrderBy = v
}

func (o QueryTracesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QueryTracesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AttributeFilters) {
		toSerialize["attribute_filters"] = o.AttributeFilters
	}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Offset) {
		toSerialize["offset"] = o.Offset
	}
	if !IsNil(o.OrderBy) {
		toSerialize["order_by"] = o.OrderBy
	}
	return toSerialize, nil
}

type NullableQueryTracesRequest struct {
	value *QueryTracesRequest
	isSet bool
}

func (v NullableQueryTracesRequest) Get() *QueryTracesRequest {
	return v.value
}

func (v *NullableQueryTracesRequest) Set(val *QueryTracesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableQueryTracesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableQueryTracesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQueryTracesRequest(val *QueryTracesRequest) *NullableQueryTracesRequest {
	return &NullableQueryTracesRequest{value: val, isSet: true}
}

func (v NullableQueryTracesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQueryTracesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


